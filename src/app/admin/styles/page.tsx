...
import { Add, Eye, EyeSlash, Save2, Setting4 } from "vuesax-icons-react";
import { Button } from "@/components/ui/button";
import { adminInputClass, adminTextareaClass, AdminMetric, AdminSection } from "@/features/admin/components/admin-ui";
      ...
      <AdminSection title="ساخت سبک جدید" eyebrow="پرامپت داخلی">
        <form action={createCreativeStyleAction} className="grid gap-3">
          <div className="grid gap-3 md:grid-cols-[1fr_120px]">
            <input name="name" placeholder="نام سبک" className={adminInputClass} />
            <input name="sortOrder" type="number" defaultValue={styles.length * 10 + 10} className={adminInputClass} />
          </div>
          <input name="description" placeholder="توضیح کوتاه برای کاربر" className={adminInputClass} />
          ...
          <textarea name="prompt" rows={4} placeholder="پرامپت داخلی سبک" className={`${adminTextareaClass} text-left`} dir="ltr" />
          ...
          {styles.map((style) => (
            <form key={style.id} action={updateCreativeStyleAction} className="grid gap-4 rounded-[var(--radius-md)] border border-border/70 bg-surface-soft/35 p-3 xl:grid-cols-[180px_minmax(0,1fr)]">
              ...
              <div className="grid gap-3">
                ...
                <textarea name="prompt" defaultValue={style.prompt} rows={7} className={`${adminTextareaClass} text-left text-xs leading-5`} dir="ltr" />
                <div className="flex flex-wrap items-center justify-between gap-2">
