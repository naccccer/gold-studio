...
export function ProjectDetailScreen({ project }: ProjectDetailScreenProps) {
  ...
  return (
    <>
      <div className="pointer-events-none fixed inset-0 -z-10 bg-[#0b0a08]" />
      <PageShell
        maxWidth="lg"
        className="relative z-10 min-h-[calc(100vh-2rem)] w-full max-w-full space-y-4 overflow-x-hidden rounded-[1.75rem] bg-[#0d0c0a] px-3 py-4 pb-20 text-surface sm:px-5"
      >
        ...
        <section className="w-full max-w-full space-y-3 overflow-hidden">
          <JewelryImageFrame
            aspect="portrait"
            className="w-full max-w-full overflow-hidden rounded-[1.45rem] border border-white/12 bg-black shadow-[0_38px_100px_-64px_rgba(0,0,0,1)] sm:min-h-[640px]"
          >
            <SafeJewelryImage
              src={resultImageSrc}
        ...
        <section className="grid w-full max-w-full gap-4 overflow-hidden">
          <div className="flex items-center gap-3">
            ...
            <div className="space-y-1">
              ...
              <p className="truncate text-xs text-surface/68">{project.title || "تصویر ورودی پروژه"}</p>
            </div>
          </div>
